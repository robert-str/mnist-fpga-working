"""
Send a test image to the FPGA for digit classification using LeNet-5.

Protocol:
  - Start marker: 0xBB 0x66 (2 bytes)
  - Image data: 784 bytes (28x28 grayscale, 8-bit signed as unsigned)
...
Preprocessing:
  - LeNet-5 uses MNIST normalization: (pixel/255 - 0.1307) / 0.3081
  - Then quantized to int8: multiply by INPUT_SCALE (127)
...
Examples:
  python send_image.py                       # Default: ../test_images/00006.png
...
def load_norm_params():
    ...
    if os.path.exists(mean_path) and os.path.exists(std_path):
        norm_mean = np.load(mean_path)[0]
        norm_std = np.load(std_path)[0]
        return norm_mean, norm_std
    else:
        print("INFO: Using default MNIST normalization parameters")
...
def main():
    ...
    # Default source is 00006.png in test_images directory (at project root)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    default_source = os.path.join(script_dir, "..", "..", "test_images", "00004.png")
